...
namespace Baku.VMagicMirror.GameInput
{
    ...
    public interface IGameInputSource
    {
        ...
        /// 首をかしげる動きは無し
        /// </summary>
        Observable<Vector2> LookAroundInput { get; }
        Observable<bool> IsCrouching { get; }
        //NOTE: I/Fの実装ではデフォルトが歩きか走りか考慮しないでOKで、切り替え指示が出てる…ということのみを通知する
        Observable<bool> IsRunWalkToggleActive { get; }
        Observable<bool> GunFire { get; }

        Observable<Unit> Jump { get; }
        Observable<Unit> Punch { get; }
        
        /// <summary> .vrma のカスタムモーションはココからkeyを指定して発火 </summary>
        Observable<string> StartCustomMotion { get; }
        /// <summary>
        /// .vrma のカスタムモーションが割当たっているボタン/キーを離すことで発火する。
        /// ただし、この発火でモーションが止まるのはループモーションのみ
        /// </summary>
        Observable<string> StopCustomMotion { get; }
    }
}
