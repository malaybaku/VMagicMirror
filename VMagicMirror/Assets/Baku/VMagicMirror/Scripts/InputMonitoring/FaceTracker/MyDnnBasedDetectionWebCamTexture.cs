...
{
    //OpenCVforUnityのうちLibFaceDetectionV3WebCamTextureExampleを削っていって解読するやつです
    public class MyDnnBasedDetectionWebCamTexture : MonoBehaviour
    {
        ...
        [Tooltip("Preprocess input image by multiplying on a scale factor.")]
        public float scale = 1.0f;

        [Tooltip("Preprocess input image by subtracting mean values. Mean values should be in BGR order and delimited by spaces.")]
        public Scalar mean = new Scalar(0, 0, 0, 0);

        [Tooltip("Indicate that model works with RGB input images instead BGR ones.")]
        ...
        //TODO: たぶん要らないのでコメントアウトしてるが、確認取ったほうが良さげ
        ...
        {
            OpenCvExtUtils.ColorsToMat(colors, rgbaMat, width, height);
            ...
            Size inpSize = new Size(
                inpWidth > 0 ? inpWidth : bgrMat.cols(),
            ...
            _outMats.Clear();
            net.forward(_outMats, outBlobNames);

            //tm.stop();
            //Debug.Log("Inference time, ms: " + tm.getTimeMilli());

            PostProcess(rgbaMat, _outMats);
        ...
        {   
            Debug.Log("Dnn Based Detection SetUp");
        ...
        private void PostProcess(Mat frame, List<Mat> outs)
        {
            ...
            for (int i = 0; i < indices.total(); ++i)
            {
                ...
                confidences.get(index, 0, confidence_arr);
                if (confidence_arr[0] > maxConf)
                {
                    maxConf = confidence_arr[0];
                    maxConfIdx = index;
                }
            }

            //普通ないはず？いやトラッキングロスするとここに来るのかな
            if (maxConfIdx == -1)
            {
            ...
            FaceRect = new Rect(bbox_arr[0], bbox_arr[1], bbox_arr[2] - bbox_arr[0], bbox_arr[3] - bbox_arr[1]);
            ...
            for (int i = 0; i < 5; i++)
            {
                LandMarks[i] = new Vector2(landmarks_arr[i * 2], landmarks_arr[i * 2 + 1]);
            }
        }
