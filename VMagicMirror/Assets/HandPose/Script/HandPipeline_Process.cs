...
partial class HandPipeline
{
    ...
    void RunDetectPalm(Texture input, float dt)
    {
        var cs = _resources.compute;
        ...
        //TODO: ProcessImageどうしの間について、前半/後半の区切りが若干難しい
        
        // Hand region bounding box update
        cs.SetFloat("_bbox_dt", dt);
        cs.SetBuffer(1, "_bbox_count", _detector.palm.CountBuffer);
        cs.SetBuffer(1, "_bbox_palm", _detector.palm.DetectionBuffer);
        cs.SetBuffer(1, "_bbox_region", _buffer.region);
        cs.Dispatch(1, 1, 1, 1);
    ...
    void RunCalculateLandmarks(Texture input, float dt)
    {
        var cs = _resources.compute;
        ...
        Profiler.BeginSample("MPHand Post Process before invalidate");
        ...
        Profiler.BeginSample("MPHand Post Process invalidate");
