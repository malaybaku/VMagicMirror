...
- [x] `詳細` Expander の見栄えとサイズ感について、ユーザーに目視チェックを依頼する。
...
- [x] `EnableWebCameraHighPowerModeLipSync` コマンド名を実態に寄せるか、既存名維持にするかを実装時に判断する。
...
- 旧 LookAt 3 bool 設定の UI は `FaceTrackerPanel` と詳細設定画面の両方から削除済み。
