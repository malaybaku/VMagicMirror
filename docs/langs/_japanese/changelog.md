...
* 追加: 表情適用中にも目ボーンを動かせるよう、設定ウィンドウの「顔・表情」>「目」>「表情きりかえ時にも目ボーンを動かし続ける」の項目を追加しました。この項目はデフォルトでは無効です。
* 変更: マイクによるリップシンクで、口の開き方が声量に応じて変化するようになりました。この処理はボリュームを大きくするか、または設定ウィンドウの「顔・表情」>「基本」>「音量でリップシンクの開き方を調整」をオフにすると従来の動きになります。
