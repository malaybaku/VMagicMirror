...
* ゲーム入力機能で使っていたVRM Animationファイルを削除した場合、そのボタンのアクション名は空欄表示になります。この状態でも使用できますが、できれば「なし」に選択状態をリセットしてください。
* ゲーム入力機能で、マウスクリック操作に対してはVRM Animationを指定しても再生されません。この問題は次バージョンで修正予定です。
