...
* 追加: webカメラで顔と手を同時にトラッキングするとクラッシュする端末があることの対策として、`ハンドトラッキング` > `クラッシュ対策モード` オプションを追加しました。このオプションをオンにするとクラッシュが起きにくくなるかわり、webカメラでは表情のトラッキングを行わなくなります。
* 変更: webカメラによる顔トラッキングについて、従来の `軽量` と `高品質` の区別を廃止して1つに統合しました。
* 削除: webカメラによる顔トラッキング時の視線について、カメラ方向を常に見る `ユーザー` オプションを廃止しました。
...
* 追加: ハンドトラッキング時に、頭の動きに手が連動するオプションを追加しました。連動の強さは `ハンドトラッキング` > `頭の動きで手も動かす [%]` から調整できます。
* 追加: 姿勢の調整として、肩の上げ方、および背中の曲げ伸ばしを調整するオプションを追加しました。詳細設定ウィンドウの `モーション` > `肩・背中` から設定できます。
* 追加: 詳細設定の `エフェクト` > `Bloom` にエフェクトのオン/オフを指定できる項目を追加しました。なお、v5.0.0以前のバージョンでは `強さ` を0にすることでエフェクトがオフの状態になります。
* 変更: フリーレイアウトモード中に表示している位置・回転の操作オブジェクトについて、内部的な生成タイミングを最適化しました。
...
* 追加: リムライト風の色彩をアバターに上乗せする機能を追加しました。詳細設定ウィンドウの `エフェクト > リムライト` から使用できます。
* 追加: 影の色とぼかし度合いを変更できるようになりました。詳細設定ウィンドウの `エフェクト > 影` から設定できます。
* 追加: ハンドトラッキング中の腕の動きについて補間処理を追加しました。
* 追加: 画質が `High` 以上の場合にHDRを有効になるようにしました。負荷を抑えたい場合、詳細設定ウィンドウの `エフェクト > 画質` からオフにできます。
* 修正: 一部モデルのロード時、腰付近のボーンが不自然にねじれてしまうことがある問題を対策しました。
* 修正: ハンドトラッキング中にトラッキングロストするとアバターの体が下方に大きく引っ張られてしまう場合があるのを対策しました。
* 変更: コントロールパネル画面の色合いを全体的に調整しました。
* 変更: 詳細設定ウィンドウから色の設定を編集するとき、独立なダイアログウィンドウ上で調整するようにしました。
* 変更: 内部的なレンダリング方法の変更として、レンダリングパイプラインをUniversal Rendering Pipeline (URP) に変更しました。
* 変更: 内部的にUniVRMバージョンを1.131.0に更新しました。
* 変更: レンダリング方法の変更に伴い、アバターの背面に表示される影の描画方法を変更しました。デフォルト設定の場合、v4.4.3に比べると描画品質が向上します。
* 削除: レンダリング方法の変更に伴い、`ゲーム入力` の使用中などに表示される足元の影の向きを設定する機能を削除しました。足元の影は `エフェクト > ライト` で指定したライトの方向に基づいてレンダリングされるようになります。
...
* 追加: 表情スイッチ機能への追加として、顔トラッキングが外れたときに適用する表情が指定できるようになりました。
* 追加: iFacialMocapで送信先アドレスを固定して使う時のために、PCのIPアドレスが確認できるようにしました。
* 変更: 内部的にUnityバージョンを6.3.13f1にアップデートしました。
...
* 追加: VRMのロード時にライセンス確認を省略するオプションを追加しました。この設定は `詳細設定ウィンドウ > ファイル` から使用できます。
* 変更: `デスクトップの色調でライトを補正` 機能の使用中に、ディスプレイ上に黄色い枠線が表示されないようにしました。この変更に伴い、 `配信` タブの `表示` から本機能を利用できるようにしました。
* 変更: ハンドトラッキング中に左右の手を逆に認識したときアバターの手が大きくクロスする動きになってしまう問題が起きにくくなる対策を追加しました。
...
* 追加: アバターウィンドウを丸/角丸に切り抜く機能を追加しました。`配信` タブの `ウィンドウ` からオン/オフできるほか、`詳細設定ウィンドウ > ウィンドウ > 形状を指定して切り抜き` メニューから表示を調整できます。
* 変更: webカメラで手を検出してトラッキングできている間は、アバターの手の動きがほかの動き (キーボード/マウス操作のモーション) に切り替わらないようになりました。
...
* 変更: webカメラでの軽量版の顔トラッキングについて、内部的に高品質版のトラッキングと同じシステムを使うようにしました。ただし、まばたき、リップシンクをカメラベースで実行できないこと等の、軽量であることに由来した制限は引き続き適用されます。
* 変更: フレームレートについて、画質の種類から自動で選択するのではなく、独立な設定として編集できるように変更しました。 `詳細設定ウィンドウ > エフェクト > 画質` から設定できます。
* 修正: v4.0.0以降での軽量版の顔トラッキング動作が意図したより素早く、ガタつきやすくなっていたのを調整しました。
* 修正: v4.2.0以降で、ゲームパッドの十字キーでアバターの傾きを制御する動作がおかしくなっていたのを修正しました。
* 修正: VMCプロトコルで受信したポーズとトラッキングしたポーズを結合して適用するオプションについて、UI上の文言が分かりづらかったのを修正しました。
...
* 追加: webカメラでのハンドトラッキング中に肘を動かせる機能を実験的に追加しました。
* 変更: webカメラでの顔トラッキングおよびハンドトラッキング時に、動きが滑らかになるような処理を内部的に追加しました。
* 変更: Unityを脆弱性について対応されたバージョン(6000.0.58f2)に更新しました。
...
* 追加: ホットキー(ショートカット)機能で、キー/マウスによる手の動作のパターンを切り替えられるようになりました。
* 追加: `詳細設定ウィンドウ > 顔・表情 > ブレンドシェイプ > 体型調整ブレンドシェイプ` の項目で、複数の体型調整ブレンドシェイプを選べるようになりました。
* 変更: アクセサリー機能で読み込む .glb からMToonのマテリアル情報を読み込めるようにしました。詳細は [アクセサリ機能でGLBファイルにMToonのマテリアルを適用して使う](../tips/use_mtoon_defined_glb) を参照下さい。
* 変更: ハンドトラッキングについて、アバターの動き方を「つねに手下げ」にしている場合もハンドトラッキングがオンであれば適用されるようにしました。
* 変更: ハンドトラッキング時の手首および指の曲がり方を調整しました。
* 変更: アバターを他のアバターに切り替えるとき、アバターの身長差に応じてカメラ位置を調整するようにしました。
* 変更: サブキャラ機能において、特定の条件下でのみライブラリの読み込みなどの高度な処理を実行できる仕組みを実験的に導入しました。
* 修正: 顔トラッキングの設定について、目のトラッキング設定に関するヘルプURLが正しく開けていなかったのを修正しました。
...
* 追加: VMC Protocolの受信機能で、下半身のポーズを受信して適用できるようになりました。
* 追加: VMC Protocolの受信時のオプションとして「受信したポーズをなめらかに適用」と「背骨・頭ボーンに受信したポーズとトラッキング結果を両方とも適用」を追加しました。
* 修正: VMC Protocolでのデータ受信中に、表情を受信していないのにwebカメラ/iFacialMocap等による表情制御も停止してしまうことがある問題を修正しました。
* 変更: ハンドトラッキング時のアバターの指について、曲げ伸ばしがはっきりと反映されやすいようにしました。
* 変更: アバターの下半身をVMC Protocol受信またはゲーム入力機能で動かしている間、シャドウをアバターの背面ではなく地面付近に表示するようにしました。この挙動は詳細設定ウィンドウの「エフェクト」>「影」から調整できます。
* 変更: iFacialMocapの受信処理など、一部の処理について内部的にパフォーマンスを改善しました。
* 廃止: VMC Protocolによるデータ受信機能から「補正なしで送信元ボーンの姿勢を適用」オプションを削除しました。v4.2.0以降では、常にこのオプションがオンの場合に相当する挙動を行います。
* 廃止: VMC Protocolによるデータ受信機能から「VMCPの受信中はカメラ機能をオフ」オプションを削除しました。カメラのオン/オフは他のオプションから自動で判断されるようになります。
...
* 追加: サブキャラのAPI機能として、テキストをセリフのように表示できる機能を追加しました。なお、このAPIはデフォルトサブキャラでは未使用です。
* 追加: サブキャラのAPI機能として、画像で表示したサブキャラにポインターが当たったりクリックを行ったりするのを検出できる機能を追加しました。
* 変更: 背景を透過したアプリケーションをドラッグ移動するとき、サブキャラの画像領域を掴んでも移動できるようにしました。
...
* 修正: v4.0.0に特有のバグとして、アプリケーションの起動時に "Load Settings" から画面が進まなくなることがあるのを対策しました。
* 修正: ハンドトラッキング時に手首の角度が急激に変化してしまう問題を修正しました。
* 修正: アバターの手首が捻れやすかった問題を対策しました。この修正で、とくにハンドトラッキング時の見栄えが改善されます。
...
* 追加: webカメラの顔トラッキングについて、高負荷なかわりにトラッキング性能が大幅に向上するオプションを導入しました。
* 追加: `サブキャラ` 機能として、通常のアバターに加えて別途キャラクターを表示できる機能を追加しました。詳細は [サブキャラ](../docs/buddy) を参照してください。
* 追加: ハンドトラッキングについて、手が動く範囲などを指定できるオプションを追加しました。
* 追加: VMC Protocolでアバターの姿勢、および表情のデータを送信できる機能を追加しました。
* 追加: 詳細設定ウィンドウの `エフェクト` タブから指定できるエフェクトとして、アンビエントオクルージョン(Ambient Occlusion)に対応しました。
* 変更: Unityバージョンを Unity 6.0.33f1 に更新しました。この変更に伴い、アプリケーション起動時にUnityロゴのスプラッシュスクリーンは表示されなくなります。
* 変更: ハンドトラッキングについて、検出範囲が広がる方向に実装を変更しました。
* 変更: VMC Protocolで受信した表情を、表情スイッチ機能の入力として考慮できるようにしました。
* 変更: Spout出力について、従来の横長解像度に加えて縦長の出力にも対応しました。ただし、縦長出力を行う場合、アバターウィンドウでは左右に黒帯がつく形でアバターが表示されます。
* 変更: 内部的な変更として、GUIとアバターウィンドウ間の通信が全体的に高速になり、かつ大容量データに対してもエラーになりにくいように実装を更新しました。
* 変更: 内部的な変更として、GUIのベースになっている .NET のバージョンを .NET 8 に更新しました。
* 修正: VRM1.0のモデルのロード時に、目ボーンのローカル座標系によっては目がおかしい方向に回転してしまう問題を修正しました。
* 修正: ドキュメントおよびUI全般で、基本的な使い方でロードされるVRMの表記を `アバター` に統一しました。この修正により、 `キャラクター` 表記は原則としてサブキャラにのみ適用されるようになります。
* 削除: v3.8.4で導入していたNLuaについて、導入を見送ったためライブラリを使用しないようにしました。
* 削除: 有償アセットのうち、OpenCVForUnity の使用を終了しました。
* 削除: ハンドトラッキングの実装変更に伴い、 BarracudaHand とそれに関するOSSの使用を終了しました。
* その他: webページについて、サブキャラの自作用の情報を提供するドキュメントのページとして [VMagicMirror Buddy Doc](https://malaybaku.github.io/VMagicMirrorBuddyDoc/) を公開しました。

なお、上記の更新のうち VMC Protocolによるデータ送信、サブキャラ、ハンドトラッキング機能はエディション差による制限があります。詳細は [ダウンロード](../download) のページを参照してください。
