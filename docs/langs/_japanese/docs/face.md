...
`視線の動き`: 視線をどう動かすか選択します。コントロールパネルの配信タブにもある機能です。`マウス`を選択すると、アバターがマウスポインターの方向を見つめます。`固定`ではマウスを無視し、基本的に正面を向きます。`ユーザー`選択は`固定`に似ていますが、`フリーカメラモード`でアバターの向きを正面向き以外にしたときも、なるべくモニターの正面方向(つまりあなた自身)を見つめるように動きます。
