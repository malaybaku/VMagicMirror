...
- `肘もトラッキングする (Experimental)`: v4.3.0 以降で利用できます。チェックをオンにすると、CPU負荷が上がる代わりに肘もトラッキングの対象になります。
...
- `頭の動きで手も動かす (%)`: ハンドトラッキング中に、顔や頭の動きに合わせて手の位置や向きも動かす強さを指定します。値を0にすると、頭の動きに対する手の追従を行いません。
