...
とくに`ロード`について、`アバターをロード`と`アバター以外をロード`のチェックがあることに注意して下さい。

`アバターをロード`のみチェックをオンにした場合、設定は現在の値のまま、アバターのみを切り替えます。同じアバターの着せ替えを切り替えたい場合に使用します。

`アバター以外をロード`をチェックした場合、アバター以外の設定としてレイアウトやトラッキング設定などがロードされます。
...
- `load_character`: 手動でロードする場合の`アバターをロード`のチェックに相当します。値を`true`または`false`で指定します。
- `load_non_character`: 手動でロードする場合の`アバター以外をロード`のチェックに相当します。値を`true`または`false`で指定します。
