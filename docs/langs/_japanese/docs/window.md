...
- `切り抜き`: 背景を透過しているときの切り抜き処理について設定します。詳細は次のセクションを参照して下さい。
...
この機能はVMM v4.4.0かそれ以降で使用できます。

本機能を用いると、円または角丸の枠で切り抜いてアバターを表示します。
...
- `(透過中) 切り抜き処理を適用`: `背景を透過` と本オプションを同時にオンにすると切り抜き処理が適用されます。デフォルトではオフになっています。このオプションは `配信` タブからも利用できます。
