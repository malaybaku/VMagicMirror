...
`VMagicMirror`はインストーラー形式で配布しています。
zipを解凍して内部のインストーラファイル(.exe)をダブルクリックで実行し、指示に従ってインストールします。
...
`VMagicMirror`を起動するとGUIがある「コントロールパネル」と、アバターが映る「アバターウィンドウ」が立ち上がります。

コントロールパネルかアバターウィンドウの一方を閉じると、もう片方の画面も閉じて`VMagicMirror`が終了します。コントロールパネルが邪魔な場合、最小化しておきます。
...
`.vrm`ファイルを選択してアバターウィンドウに現れる規約を確認します。`OK`をクリックするとアバターがロードされます。
...
ロードしたアバターを次回以降も使いたい場合、`ホーム`タブの`次回の起動時にも同じVRMを読み込む`のチェックをオンにします。

<div class="row">
{% include docimg.html file="./images/get_started/img00_040_after_loaded.jpg" customclass="col s12 m6" imgclass="fit-doc-img" %}
</div>

また、この時点でキーボードやタッチパッドの位置、視点がアバターに合わない場合、`アバター体格で補正`ボタンをクリックします。
...
v4.4.0以降のバージョンでは、背景が透明、かつ`(透過中) 丸/角丸に切り抜く`をオンにすると切り抜き処理を適用した透過表示を行えます。

背景が透明であり、かつ`(透過中) アバター付近を掴んでドラッグ`のチェックがオンであれば、アバターを`左クリック+ドラッグ`で移動できます。`(透過中)アバター付近を掴んでドラッグ`をオフにすると、アバターがクリックに反応しなくなり、背面のアプリケーションをクリックできます。
...
- `顔をトラッキング`: ウェブカメラを選択することで、頭部の動作を反映します。

</div>

顔をトラッキングさせてもモデルの首が回らない場合、[FAQ](../questions)の「顔トラッキングで首が回らない」を参照下さい。

顔トラッキングのより高度な設定は `顔トラッキング` タブから利用でき、webカメラ以外でiOSデバイス等との連携もサポートしています。詳しくは [顔トラッキング](../docs/face_tracker) を参照ください。
...
`表示`ではアバター以外のデバイス表示やエフェクトのオン・オフを切り替えます。
...
アバターの影の見栄えが悪い場合、[FAQ](../questions)の「影が綺麗に映らない」の項目を確認してください。
...
カメラ機能では、アバターをうつす視点を操作できます。

本機能を使うときは通常`ウィンドウ`メニューの`背景を透過`をオフにします。その後、`フリーカメラモード`チェックをオンにすると、アバターウィンドウ上で視点を動かせます。
...
アバターを見失った場合や始めからやり直す場合、`位置をリセット`ボタンで初期状態に戻します。
...
2. `(透過中)アバター付近を掴んでドラッグ`をオンにする
3. アバターを左クリックし、各操作で視点を調整
4. 調整後、`(透過中)アバター付近を掴んでドラッグ`と`フリーカメラモード`をオフにする

</div>

ただし、この方法ではアバターが表示ウィンドウの外に見切れやすいです。

アバターを見失った場合は`位置をリセット`ボタンでリセットして下さい。
...
とくに`デバイスの割り当て`で`ゲームパッド`や`MIDIコントローラ`を選ぶことで、アバターの動作に反映させずに表情を切り替えられます。
